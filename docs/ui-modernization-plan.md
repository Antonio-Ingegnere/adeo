...
**S1 withdrawn.** The 680px cap was the only thing bounding the field, which is why its right edge landed arbitrarily; a later 860px cap merely moved the arbitrary point. The field now has no `max-width` at all — it is bounded by the content column on the left and a fixed-width trailing group on the right, which makes it exactly as wide as the add-task input at every window size (verified at 600/1280/1600px). Any line length too long to scan here is equally too long in the add-task input, so it is a problem to fix in both rows or neither, not asymmetrically in one.

**S15 — row symmetry.** The search row and the add-task row have the same tripartite structure (wide input · medium control · round button) and already shared their outer bounds, but their internal seams missed by 29px and 33px — close enough to read as broken alignment rather than as deliberate difference. The search row's trailing group is now pinned to 181px = the add-task row's `min-width:140px` picker + 8px gap + 33px button, so all four seams coincide. Two deliberate asymmetries remain: the `?` is 26px against the `+`'s 33px (tertiary help must not carry the same weight as the app's primary verb), and the switch is not forced to the picker's width (a mode is not a value). The alignment drifts when a list name grows the picker past 140px — measured at 109px for a 32-character name — which was accepted over capping the picker and truncating list names.
